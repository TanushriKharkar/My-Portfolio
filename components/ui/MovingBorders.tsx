...
export const MovingBorder = ({
    ...
    const pathRef = useRef<SVGRectElement | null>(null); // Properly typed ref
    const progress = useMotionValue<number>(0);
  
    useAnimationFrame((time) => {
      const length = pathRef.current?.getTotalLength(); // Safely access ref
      if (length) {
    ...
    const x = useTransform(
      progress,
      (val) => pathRef.current?.getPointAtLength(val)?.x || 0 // Fallback to 0
    );
    const y = useTransform(
      progress,
      (val) => pathRef.current?.getPointAtLength(val)?.y || 0 // Fallback to 0
    );
      ...
      <>
        ...
        >
          <rect
            ...
            rx={rx}
            ry={ry}
            ref={pathRef} // Attach ref to the SVG rect
          />
        </svg>
        <motion.div
          style={{
